// import "./globals.css";

const Binance = () => {
  return (
    <div className="bg-[#541d69]  h-[554px] lg:h-[738px] xl:h-[640px]  text-center  text-white  relative overflow-hidden">
      {/* <div className="absolute right-2 top-3 custom-radial-gradient-purpleWhite opacity-60  z-0"></div>
      <div className="absolute -left-20 -top-20 custom-radial-gradient-Whitepurple opacity-60  z-0"></div>
      <div className="absolute -bottom-10 -left-10 custom-radial-gradient-Whitepurple opacity-60  z-0"></div> */}

      {/* <div className="absolute right-0 bottom-0 custom-radial-gradient4  opacity-60 z-0"></div>
      <div className="absolute -left-24 md:-left-10 bottom-0 custom-radial-gradient3  opacity-60 z-0"></div>
      <div className="custom-radial-gradient rounded-full inset-0 backdrop-filter backdrop-blur-lg absolute -top-20 -left-20"></div> */}

      {/* <div className="bg-[#b628aa73] my-6 w-[300px] h-[500px] xl:h-[550px] xl:w-[1200px] mx-auto xl:my-10 rounded-3xl z-20"></div> */}
      <div
        className="bg-[#01204c] mx-auto z-20
        w-[85%] h-[100%] md:w-[100%]"
      ></div>

     <div className="">
      <h1 className="top-[30%] left-[15%] text-3xl md:top-[30%] md:left-[15%] md:text-6xl lg:top-[25%] lg:left-[17%] lg:text-7xl xl:text-7xl  xl:top-40 xl:left-96 text-white font-bold font-serif subhead  absolute z-10 sb">
        ON BINANCE
      </h1>

      <ul className="  top-[39%] left-[25%] text-sm md:top-[43%] md:left-[25%] md:text-xl lg:top-[37%] lg:left-[25%] lg:text-2xl xl:text-2xl  xl:top-[40%] xl:left-[36%]  text-white font-light  absolute font-mono list-disc  leading-relaxed">
        <li>Never Ending Algorithm.</li>
        <li>Secure Smart Contract. </li>
      </ul>
      </div>


      <img
        src="./0007-removebg-preview.png"
        className="w-20 absolute top-20 left-40 md:top-40 md:left-16 lg:w-40 animate-spin-slow z-0"
      />


      <img
        src="./0007-removebg-preview.png"
        className="md:hidden w-20 absolute top-20 left-40 md:top-40 md:left-16 animate-spin-slow z-0"
      />

     

      <img
        src="./0006-removebg-preview.png"
        className="w-40 absolute top-80 lg:mr-20 -right-10 md:top-36 md:w-64 lg:w-96 lg:-right-0 xl:w-[250px] animate-spin-slow2"
      />

      
    </div>
  );
};

export default Binance;

{
  /* <img src="./0003-removebg-preview.png" className="w-60 absolute top-60" /> */
}
{
  /* <img
        src="./0005-removebg-preview.png"
        className="w-80 absolute top-10 right-10"
      /> */
}
{
  /* <img
        src="./0003-removebg-preview.png"
        className="w-72 absolute bottom-0 right-28"
      /> */
}
{
  /* <img
        src="./0004-removebg-preview.png"
        className="w-60 absolute -bottom-10"
      /> */
}

{
  /* <div className="custom-radial-gradient rounded-full inset-0 backdrop-filter backdrop-blur-lg absolute -top-20 -left-16"></div> */
}

{
  /* <img src="./0001-removebg-preview.png" className="w-60 absolute top-0" /> */
}

{
  /*<img src="./0019-removebg-preview.png" className="w-60 absolute top-0" />*/
}
{
  /* <img src="./0001-removebg-preview.png" className="w-60 absolute top-0" /> */
}

{
  /* <div className="custom-radial-gradient rounded-full inset-0 backdrop-filter backdrop-blur-lg absolute bottom-0 left-0"></div> */
}
